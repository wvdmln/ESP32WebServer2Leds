...
const char* ssid = "snuffie";  // vervang!!
const char* password = "eiffuns3473";  // vervang!!
...
void setup() {
  ...
  Serial.println ("ESP32WebServer2Leds");
  ...
  // Connecteer Wi-Fi network met SSID en paswoord
  ...
  // Print het IP address en start web server
  Serial.println("");
...
void loop(){
  ...
  if (client) {                             // Als nieuwe client connecteert,
    ...
    while (client.connected() && currentTime - previousTime <= timeoutTime) {  // loop while zo lang de client geconnecteerd is
      currentTime = millis();
